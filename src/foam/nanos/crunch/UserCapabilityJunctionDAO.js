  ...
  methods: [
    ...
    },
    {
      ...
      javaCode: `
        ...
        AuthService auth = (AuthService) x.get("auth");
        if ( old != null && ucj.getSourceId() != old.getSourceId() && ! auth.check(x, SOURCE_CAPABILITY_ID_CHANGE) ) throw new RuntimeException(this.ERROR_TWO + old.getId());
        if ( old != null && ! ucj.getTargetId().equals(old.getTargetId()) && ! auth.check(x, TARGET_CAPABILITY_ID_CHANGE) ) throw new RuntimeException(this.ERROR_THREE + old.getId());
