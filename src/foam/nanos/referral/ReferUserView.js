  ...
  methods: [
    function init() {
      this.refLink = this.subject.user.referralCodes.where(
        ...
        if ( ! v.array[0] ) {
          // if no autogenerated referralcodes available, select the first referral code
          this.subject.user.referralCodes.limit(1).select().then(arr => {
            this.refLink = arr.array[0]?.url;
