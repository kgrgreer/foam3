  ...
  methods: [
    {
      ...
      javaCode: `
        ...
        Account destinationAccount = quote.getSourceAccount();
    
        // Check if AFEX can handle this transaction
        if ( ! (sourceAccount instanceof BankAccount) || ! (destinationAccount instanceof BankAccount) ) return null;
    
        // Check if AFEXTransactionPlanner can handle the currency combination
        FXService fxService = CurrencyFXService.getFXServiceByNSpecId(x, requestTxn.getSourceCurrency(),
          requestTxn.getDestinationCurrency(), AFEX_SERVICE_NSPEC_ID);
        if ( fxService instanceof AFEXServiceProvider  ) {
          ...
          return quote.getRequestTransaction();
        }
        return null;
      `
    },
