...
public class BlacklistTest extends Test {
  ...
  public void runTest(X x) {  
    ...
    try {
      transactionDAO.inX(busAdminContext).put(transaction);
      test(false, "Transaction successfully put to the TransactionDAO without passing proper compliance.");
    } catch (AuthorizationException ae) {
      test(true, "Threw auth exception as expected when compliance is not set properly: " + ae);
    } catch (Throwable t) {
      t.printStackTrace();
    ...
    try {
      transactionDAO.inX(busAdminContext).put(transaction);
      test(true, "Successfully put the transaction to the TransactionDAO after setting compliance to passed.");
    } catch (Throwable t) {
