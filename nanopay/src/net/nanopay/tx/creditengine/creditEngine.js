  ...
  methods: [
    {
      ...
      javaCode: `
        if ( obj != null && obj instanceof Transaction ) {
          if (t instanceof SummarizingTransaction) {
            return obj;
          }

          //-- specific credit
          CreditCode creditCode = t.findCreditCode(x);
          ArrayList<CreditLineItem> credits = new ArrayList<CreditLineItem>();

          String fee;
          if ( ! SafetyUtil.isEmpty(creditCode.getFee()) ) {
            fee = creditCode.getFee();
          }
          Transaction t = (Transaction) obj;
          for (TransactionLineItem tli : t.getLineItems()) {
            if ( tli instanceof FeeLineItem ) {
              FeeLineItem fli = (FeeLineItem) tli;
              if ( SafetyUtil.equals(fli.getFeeRule(), fee) ) {
                CreditLineItem cli = new CreditLineItem();
                // fill in details.. add to credit array..
                cli.setAmount((Long) (creditCode.getDiscountPercentage() * tli.getAmount()) );
        ...
        if ( obj != null && obj instanceof CreditCode ) {
