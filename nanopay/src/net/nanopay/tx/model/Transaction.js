...
`Unknown property foam.core.Currency.tableCellFormatter: function (fees){
        var formattedFees = fees / 100;
        this.start().add('$', formattedFees.toFixed(2)).end()
      }`,
`Unknown property foam.core.Double.tableCellFormatter: function (rate){
        this.start().add(rate.toFixed(2)).end()
      }`,
`Unknown property foam.core.Currency.tableCellFormatter: function (receivingAmount) {
        this.start({ class: 'foam.u2.tag.Image', data: 'images/india.svg' })
            .add(' INR ', ( receivingAmount/100 ).toFixed(2))
      }`,
`Unknown property foam.core.Currency.tableCellFormatter: function (amount) {
        this.start({ class: 'foam.u2.tag.Image', data: 'images/canada.svg' })
            .add(' CAD ', ( amount/100 ).toFixed(2))
      }`,
`Unknown property foam.core.Long.tableCellFormatter: function (payeeId, X) {
        var self = this;
...
`Unknown property foam.core.Long.tableCellFormatter: function (payerId, X) {
        var self = this;
