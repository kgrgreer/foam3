            ...
            for (Object applicableFee : applicableFees) {
              ...
              if ( feeAccount > 0 ) {
                FeeLineItem[] forward = new FeeLineItem [] {
                  new FeeLineItem.Builder(x).setNote(fee.getName()).setFeeAccount(fee.getFeeAccount()).setAmount(fee.getFee().getFee(transaction.getAmount())).setSourcePaysFees(fee.setSourcePaysFees()).build()
                };
