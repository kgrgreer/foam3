      ...
      buildJavaClass: function(cls) {
        cls.extras.push(`
          ...
          static public TrustAccount find(X x, Account account, String institutionNumber) {
            ...
            List<Institution> institutions = ((ArraySink) ((DAO) x.get("institutionDAO"))
              .where(EQ(Institution.INSTITUTION_NUMBER,institutionNumber))
              .select(new ArraySink())).getArray();
            Institution institution = null;
            if ( institutions.size() > 0 ) {
              institution = institutions.get(0);
              if ( institutions.size() > 1 ) {
                logger.warning("Multiple Institutions founnd for institution number", institutionNumber);
              }
            } else {
              logger.error("Institution not found", institutionNumber);
              throw new RuntimeException("Institution not found: "+institutionNumber);
            }
