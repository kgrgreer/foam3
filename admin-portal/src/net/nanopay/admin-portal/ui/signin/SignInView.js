foam.CLASS({
  package: 'net.nanopay.admin.ui.signin',
  name: 'SignInView',
      ...
      .start()
        ...
        .start('div')
          ...
          .start('p').addClass('link2')
            .add("Forgot Password?")
            .on('click', function(){ self.stack.push({ class: 'net.nanopay.admin.ui.forgotPassword.EmailView' })})
          .end()
        .end()
      .end()
    }
  ...
  listeners: [
    function signUp(){
      ...
      var view = foam.u2.ListCreateController.CreateController.create(
        ...
        this.__context__.createSubContext({
          detailView: net.nanopay.admin.ui.registration.UserRegistrationView,
    ...
    function signIn(){
      ...
      this.userDAO.where(this.AND(this.EQ(this.User.EMAIL, this.email), this.EQ(this.User.PASSWORD, this.password))).select().then(function(user){
        ...
        self.stack.push({ class: 'net.nanopay.admin.ui.dashboard.DashboardView' })
      })
    }
  ],
});