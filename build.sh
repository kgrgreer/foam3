...
function start_nanos {
    if [ "${RUN_JAR}" -eq 1 ]; then
        ...
        ${NANOPAY_HOME}/bin/run.sh -Z${DAEMONIZE} -D${DEBUG} -S${DEBUG_SUSPEND} -P${DEBUG_PORT} -N${NANOPAY_HOME} -C${CLUSTER} -H${HOST_NAME} -j${PROFILER} -J${PROFILER_PORT} -F${FS} ${OPT_ARGS}
    else
        cd "$PROJECT_HOME"
