...
import Foundation
public class ArrayParser: ProxyParser {
...
super.set(key: key, value: value)
  ...
  private class ClassInfo_: ClassInfo {
    ...
    lazy var id: String = "foam.swift.parse.json.ArrayParser"

    lazy var label: String = "Array Parser"

    lazy var parent: ClassInfo? = ProxyParser.classInfo()

    lazy var ownAxioms: [Axiom] = [DELEGATE]

    lazy var cls: AnyClass = ArrayParser.self

    func `create`(args: [String:Any?] = [:], x: Context) -> Any {
      return ArrayParser(args, x)
    }
