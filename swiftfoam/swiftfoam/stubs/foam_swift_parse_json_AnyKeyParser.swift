...
import Foundation
public class AnyKeyParser: ProxyParser {
...
return Alt(["parsers": [
  ...
  public override func `ownClassInfo`() -> ClassInfo {
    return AnyKeyParser.classInfo_
  }
...
super.set(key: key, value: value)
  ...
  private class ClassInfo_: ClassInfo {
    ...
    lazy var id: String = "foam.swift.parse.json.AnyKeyParser"

    lazy var label: String = "Any Key Parser"

    lazy var parent: ClassInfo? = ProxyParser.classInfo()

    lazy var ownAxioms: [Axiom] = [DELEGATE]

    lazy var cls: AnyClass = AnyKeyParser.self

    func `create`(args: [String:Any?] = [:], x: Context) -> Any {
      return AnyKeyParser(args, x)
    }
