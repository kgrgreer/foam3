...
import Foundation
public class ExprParser: ProxyParser {
...
super.set(key: key, value: value)
  ...
  private class ClassInfo_: ClassInfo {
    ...
    lazy var id: String = "foam.swift.parse.json.ExprParser"

    lazy var label: String = "Expr Parser"

    lazy var parent: ClassInfo? = ProxyParser.classInfo()

    lazy var ownAxioms: [Axiom] = [DELEGATE]

    lazy var cls: AnyClass = ExprParser.self

    func `create`(args: [String:Any?] = [:], x: Context) -> Any {
      return ExprParser(args, x)
    }
