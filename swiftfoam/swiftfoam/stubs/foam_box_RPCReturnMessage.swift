...
import Foundation
public class RPCReturnMessage: Message {
...
self.onDetach(self.data_Value_Sub_!)
  ...
  public override func `ownClassInfo`() -> ClassInfo {
    return RPCReturnMessage.classInfo_
  }
...
super.set(key: key, value: value)
  ...
  private class ClassInfo_: ClassInfo {
    ...
    lazy var id: String = "foam.box.RPCReturnMessage"

    lazy var label: String = "RPCReturn Message"

    lazy var parent: ClassInfo? = Message.classInfo()

    lazy var ownAxioms: [Axiom] = [DATA]

    lazy var cls: AnyClass = RPCReturnMessage.self

    func `create`(args: [String:Any?] = [:], x: Context) -> Any {
      return RPCReturnMessage(args, x)
    }
