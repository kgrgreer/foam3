...
import Foundation
public class NullParser: ProxyParser {
...
self.set(key: "delegate", value: value)
  ...
  private func `_delegate_factory_`() -> Parser {
    return Literal(["string": "null", "value": nil])
  }
...
super.set(key: key, value: value)
  ...
  private class ClassInfo_: ClassInfo {
    ...
    lazy var id: String = "foam.swift.parse.json.NullParser"

    lazy var label: String = "Null Parser"

    lazy var parent: ClassInfo? = ProxyParser.classInfo()

    lazy var ownAxioms: [Axiom] = [DELEGATE]

    lazy var cls: AnyClass = NullParser.self

    func `create`(args: [String:Any?] = [:], x: Context) -> Any {
      return NullParser(args, x)
    }
