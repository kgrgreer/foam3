...
import Foundation
public class DateParser: ProxyParser {
...
super.set(key: key, value: value)
  ...
  private class ClassInfo_: ClassInfo {
    ...
    lazy var id: String = "foam.swift.parse.json.DateParser"

    lazy var label: String = "Date Parser"

    lazy var parent: ClassInfo? = ProxyParser.classInfo()

    lazy var ownAxioms: [Axiom] = [DELEGATE]

    lazy var cls: AnyClass = DateParser.self

    func `create`(args: [String:Any?] = [:], x: Context) -> Any {
      return DateParser(args, x)
    }
