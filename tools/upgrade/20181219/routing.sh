...
if [ -f "$JOURNAL_HOME/services.0" ]; then
    perl -p -i -e 's/^/p({"class":"foam.nanos.boot.NSpec", "name":"routingJournal",  "lazy":false,  "serve":false,  "serviceScript":"new foam.dao.RoutingJournal.Builder(x).setFile(\"journal\").build()"})\n/' "$JOURNAL_HOME"/services.0
